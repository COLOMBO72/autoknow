...
function GuideSection({ title, children }) {
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      <div style={{ fontSize: 13.5, lineHeight: 1.7, color: tokens.inkSoft }}>
        {children}
      </div>
    </div>
  );
  ...
  return (
    <Card>
      <p style={{ fontSize: 13, color: tokens.inkSoft, marginBottom: 20 }}>
        Памятка для тех, кто покупает или продаёт автомобиль впервые. Отчёты
        сервиса дополняют это — сами по себе не заменяют осмотр и диагностику.
      </p>

      <GuideSection title="ДКП — договор купли-продажи">
        <p>
          Обязательно должен содержать: ФИО и паспортные данные обеих сторон;
          данные автомобиля (марка, модель, VIN, гос. номер, год выпуска); цену;
          дату и место составления; подписи сторон.
        </p>
        <p style={{ marginTop: 8 }}>
          Можно написать от руки в свободной форме или использовать типовой
          бланк — юридическую силу это не меняет. Один экземпляр остаётся у
          каждой стороны, нужен для постановки на учёт в ГИБДД.
        </p>
      </GuideSection>

      <GuideSection title="Что проверить при осмотре">
        <p>
          Кузов — сколы, следы недавней покраски (разнотон, неровные зазоры
          между панелями), ржавчина в арках и порогах.
        </p>
        <p style={{ marginTop: 8 }}>
          Двигатель — подтёки масла, цвет выхлопа на холодном запуске,
          посторонние стуки.
        </p>
        <p style={{ marginTop: 8 }}>
          VIN и номера агрегатов — сверить с указанными в ПТС/СТС, не должно
          быть следов перебивки.
        </p>
        <p style={{ marginTop: 8 }}>
          История — проверить на Госуслугах или через ГИБДД: залог, участие в
          ДТП, ограничения, количество предыдущих владельцев.
        </p>
        <p style={{ marginTop: 8 }}>
          Перед покупкой — обязательна диагностика на независимом СТО, не у
          продавца.
        </p>
      </GuideSection>
      ...
      <GuideSection title="Как понять рыночную цену">
        <p>
          Сравни несколько актуальных объявлений на авито/дром для машин того же
          года, пробега и региона — не ориентируйся на одно объявление.
        </p>
        <p style={{ marginTop: 8 }}>
          Учитывай, что цена в объявлении обычно чуть выше той, за которую
          реально продают — торг закладывается заранее.
        </p>
        <p style={{ marginTop: 8 }}>
          Медиана в наших отчётах — это ориентир по рынку в целом, а не цена
          конкретного экземпляра: состояние, комплектация и торг всё равно
          двигают её в ту или иную сторону.
        </p>
      </GuideSection>
    </Card>
  );
    ...
    return (
      <Card>
        <p style={{ fontSize: 14, color: tokens.inkSoft, marginBottom: 14 }}>
          Сейчас доступ гостевой — без email и пароля. Зарегистрируйся, чтобы не
          потерять баланс и историю при смене устройства.
        </p>
        <a
          href="/login"
          style={{
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      {msg && (
        <p
          style={{
            color: msg.type === "ok" ? tokens.ink : tokens.red,
            fontSize: 13,
          }}
        >
          {msg.text}
      ...
      <Card>
        ...
        <p style={{ fontSize: 12.5, color: tokens.inkSoft, marginBottom: 10 }}>
          Сейчас: {currentEmail || "—"}
        </p>
          ...
          <>
            ...
            >
              <div>
                ...
                <div
                  style={{ fontSize: 12, color: tokens.inkSoft, marginTop: 4 }}
                >
                  ≈ {Math.floor(balance / 7900)} отчёта по текущей цене
                </div>
              </div>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                {PACKAGES.map((p) => (
                  ...
                  >
                    ...
                    >
                      {p.reports} {p.reports === 1 ? "отчёт" : "отчётов"}
                    </div>
                  </div>
                ))}
              </div>
            ...
            {getUserEmail() &&
              getUserEmail() === process.env.NEXT_PUBLIC_ADMIN_EMAIL && (
                <a
                  href="/admin"
                  style={{
                    display: "flex",
              ...
              {TABS.map((t) => (
                <button
                  ...
                  style={{
                    background: "none",
                    border: "none",
                    borderBottom: `2px solid ${tab === t.id ? tokens.red : "transparent"}`,
              ...
              <Card>
                {reports.length === 0 && (
                  <p style={{ color: tokens.inkSoft, fontSize: 14 }}>
                    Пока пусто — купленные отчёты появятся здесь.
                  </p>
                )}
                {reports.map((r) => (
                  <ListRow
                    ...
                    right={
                      <a
                        href={`/report?brand=${r.carVariant.brand}&model=${r.carVariant.model}&yearFrom=${r.carVariant.yearFrom}`}
                        className="ak-link"
            ...
            {tab === "comparisons" && (
              <Card>
                {comparisons.length === 0 && (
                  <p style={{ color: tokens.inkSoft, fontSize: 14 }}>
                    Пока пусто — сравнения появятся здесь.
                  </p>
                )}
                {comparisons.map((c) => (
                  <ListRow
                    key={c.id}
                    title={`Сравнение ${c.carVariantIds.length} машин`}
