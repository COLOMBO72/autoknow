...
function carFromParams(sp) {
  const car = {
    brand: sp.get("brand"),
    model: sp.get("model"),
    yearFrom: Number(sp.get("yearFrom")),
  };
  ...
  if (!car.brand || !car.model || !car.yearFrom) {
    return shell(
      <p style={{ color: tokens.inkSoft }}>
        Не хватает параметров машины.{" "}
        <a href="/" className="ak-link">
          Вернуться к выбору →
        </a>
      </p>,
    );
  ...
  if (state.error) {
      ...
      return shell(
        <div style={{ padding: "60px 0", textAlign: "center" }}>
          <p style={{ fontSize: 16, marginBottom: 20 }}>
            Не хватает баланса, чтобы показать это досье.
          </p>
          <a
            href="/account"
    ...
    return shell(
      <div style={{ padding: "40px 0" }}>
        <p style={{ color: tokens.red, marginBottom: 12 }}>
          Не получилось получить отчёт: {state.error.message}
        </p>
        <a href="/" className="ak-link">
          ← Назад к выбору
        </a>
    ...
    <>
      ...
      >
        <StatTile
          ...
          value={fmtRub(r.price.marketPriceRub.median)}
          delay={0}
        />
        <StatTile
          label="Топливо + ТО в год"
          value={`${fmtRub(r.costs.fuelPerYearRub.min + r.costs.maintenancePerYearRub.min)}–${fmtRub(r.costs.fuelPerYearRub.max + r.costs.maintenancePerYearRub.max)}`}
          delay={100}
        />
        <StatTile
          label="ОСАГО"
          value={`${fmtRub(r.insurance.osagoPerYearRub.min)}–${fmtRub(r.insurance.osagoPerYearRub.max)}`}
          delay={200}
        />
      </div>
            ...
            {r.specs.engines.map((e) => (
              ...
              >
                <div>
                  ...
                  >
                    {FUEL_LABEL[e.fuelType] ?? e.fuelType} ·{" "}
                    {e.transmissionOptions.join(", ")}
                  </div>
                  {e.overhaulMileageKm && (
                    <div
                      style={{
                    ...
                    >
                      ...
                      {Math.round(e.overhaulMileageKm.min / 1000)}–
                      {Math.round(e.overhaulMileageKm.max / 1000)} тыс. км
                    </div>
                  )}
                </div>
                <div
                ...
                return (
                  ...
                  >
                    ...
                    {issue.mileageOrAgeHint && (
                      ...
                      >
                        ⌁ обычно на {issue.mileageOrAgeHint}
                      </p>
                    )}
                  </div>
                );
      ...
      <section style={{ marginBottom: 36 }}>
        ...
        >
          <div>
            ...
            <RangeBar
              min={r.costs.fuelPerYearRub.min}
              max={r.costs.fuelPerYearRub.max}
              value={r.costs.fuelPerYearRub.max}
            />
          </div>
          <div>
            <div
            ...
            <RangeBar
              min={r.costs.maintenancePerYearRub.min}
              max={r.costs.maintenancePerYearRub.max}
              value={r.costs.maintenancePerYearRub.max}
            />
          </div>
          <div
            style={{
          ...
          >
            <span style={{ fontSize: 12, color: tokens.inkSoft }}>
              ...
              <strong style={{ color: tokens.ink }}>
                {PARTS_LABEL[r.costs.partsAvailability]}
              </strong>
              . {r.costs.partsNote}
            </span>
          </div>
      ...
      <section style={{ marginBottom: 36 }}>
        ...
        >
          <div>
            <div
              style={{ fontSize: 12, color: tokens.inkSoft, marginBottom: 4 }}
            >
              ОСАГО / год
            </div>
            <div
            ...
            >
              {fmtRub(r.insurance.osagoPerYearRub.min)} –{" "}
              {fmtRub(r.insurance.osagoPerYearRub.max)}
            </div>
          </div>
          {r.insurance.kaskoPerYearRub && (
            <div>
              ...
              >
                {fmtRub(r.insurance.kaskoPerYearRub.min)} –{" "}
                {fmtRub(r.insurance.kaskoPerYearRub.max)}
              </div>
            </div>
          )}
          <div
      ...
      <section style={{ marginBottom: 36 }}>
        ...
        <Card>
          ...
          >
            ...
            >
              {fmtRub(r.price.marketPriceRub.median)}
            </div>
            <div style={{ fontSize: 13, color: tokens.inkSoft }}>
              медиана · диапазон {fmtRub(r.price.marketPriceRub.min)} –{" "}
              {fmtRub(r.price.marketPriceRub.max)}
            </div>
          </div>
          <RangeBar
            min={r.price.marketPriceRub.min}
            max={r.price.marketPriceRub.max}
            value={r.price.marketPriceRub.median}
          />
          <p style={{ fontSize: 12.5, color: tokens.inkSoft, marginTop: 14 }}>
            По состоянию на {r.price.asOfDate}. {r.price.depreciationNote}
          </p>
        </Card>
      </section>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        <button
          ...
          onClick={() => {
            addToCart({
              ...car,
              title: `${car.brand} ${car.model} ${car.yearFrom}`,
            });
          ...
          style={{
            ...
            borderRadius: 8,
            border: "none",
            background: addedToCompare ? tokens.line : tokens.red,
            color: "#fff",
            cursor: addedToCompare ? "default" : "pointer",
          }}
        >
      ...
      <div style={{ marginTop: 32 }}>
        <FeedbackWidget
          userId={userId}
          carVariantId={state.data.carVariantId}
        />
      </div>
    </>,
  );
...
export default function ReportPage() {
  return (
    <Suspense
      fallback={<div style={{ minHeight: "100vh", background: "#0B0C0E" }} />}
    >
      <ReportContent />
    </Suspense>
  );
