...
async function request(path, options = {}) {
  ...
  if (!res.ok) {
    ...
    const error = new Error(
      body?.message || `Бэкенд ответил ошибкой на ${path} (${res.status})`,
    );
...
export const api = {
  ...
  getKnownVariants: (brand, model) =>
    request(
      `/catalog/${encodeURIComponent(brand)}/${encodeURIComponent(model)}/variants`,
    ),
  purchaseReport: (userId, car) =>
    request("/reports/purchase", {
      method: "POST",
      body: JSON.stringify({ userId, car }),
    }),
  compareReports: (userId, cars) =>
    request("/reports/compare", {
      method: "POST",
      body: JSON.stringify({ userId, cars }),
    }),
  topup: (userId, amountKopeks) =>
    request("/billing/topup", {
      method: "POST",
      body: JSON.stringify({ userId, amountKopeks }),
    }),

  register: (email, password, existingUserId) =>
    request("/auth/register", {
      method: "POST",
      body: JSON.stringify({ email, password, existingUserId }),
    }),
